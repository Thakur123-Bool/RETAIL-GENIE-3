...
DFS_VERSION = os.getenv("DFS_VERSION", "2023-11-03")
MAX_RESULTS = int(os.getenv("MAX_RESULTS", "5000"))
REQUEST_TIMEOUT = int(os.getenv("REQUEST_TIMEOUT", "30"))  # default timeout for API calls
...
PIPELINE_POLL_INTERVAL = int(os.getenv("PIPELINE_POLL_INTERVAL", "6"))
PIPELINE_POLL_TIMEOUT = int(os.getenv("PIPELINE_POLL_TIMEOUT", "1800"))
MODEL_POLL_INTERVAL = int(os.getenv("MODEL_POLL_INTERVAL", "4"))
MODEL_POLL_ATTEMPTS = int(os.getenv("MODEL_POLL_ATTEMPTS", "120"))
...
def get_token(tenant, client, secret, scope):
    ...
    try:
        ...
        raise RuntimeError("Azure AD token request timed out. Check network/credentials.")
    except Exception as e:
        raise RuntimeError(f"Azure AD token error: {str(e)}")
...
def ado_list_items(org, project, repo, path, pat):
    ...
    url = f"https://dev.azure.com/{org}/{project}/_apis/git/repositories/{repo}/items"
    params = {"scopePath": path, "recursionLevel": "Full", "includeContent": "false", "api-version": api_version}
    r = requests.get(url, params=params, auth=HTTPBasicAuth('', pat), timeout=30)
    if r.status_code >= 300:
        raise RuntimeError(f"ADO list failed: {r.status_code} - {r.text[:1000]}")
    return r.json().get("value", [])
...
def trigger_pipeline(fabric_token, params=None):
    ...
    try:
        ...
        raise RuntimeError("Trigger pipeline request timed out.")
    if r.status_code != 202:
        raise RuntimeError(f"Pipeline trigger failed: {r.status_code} - {r.text[:2000]}")
    loc = r.headers.get("Location")
    run_id = loc.rstrip("/").split("/")[-1]
...
def poll_pipeline(fabric_token, run_id, timeout=PIPELINE_POLL_TIMEOUT, interval=PIPELINE_POLL_INTERVAL):
    ...
    while waited < timeout:
        ...
        if r and r.status_code == 200:
            ...
            log.warning("pipeline status fetch returned %s", getattr(r, "status_code", "no response"))
        time.sleep(interval)
        waited += interval
    raise RuntimeError("Pipeline poll timed out")
...
def upload_parts_to_fabric(fabric_token, display_name, parts):
    ...
    try:
        ...
        raise RuntimeError(f"Semantic upload request failed: {str(e)}")
    if r.status_code not in (200, 201, 202):
        raise RuntimeError(f"Upload failed: {r.text[:2000]}")
    status_url = r.headers.get("Location") or r.headers.get("location")
    if not status_url:
    ...
    for _ in range(MODEL_POLL_ATTEMPTS):
        time.sleep(MODEL_POLL_INTERVAL)
        try:
            pr = requests.get(status_url, headers=headers, timeout=20)
        except Exception:
            continue
        if pr.status_code != 200:
            continue
...
def find_pbi_dataset(pbi_token, name):
    url = f"{PBI_BASE}/groups/{WORKSPACE_ID}/datasets"
    try:
        r = requests.get(url, headers={"Authorization": f"Bearer {pbi_token}"}, timeout=20)
    except Exception as e:
        raise RuntimeError(f"Dataset search failed: {str(e)}")
    if r.status_code != 200:
        raise RuntimeError(f"Failed to list datasets: {r.status_code}")
    for ds in r.json().get("value", []):
        if ds.get("name") == name:
...
def clone_pbi_report(pbi_token, template_id, dataset_id, new_name):
    base_url = f"{PBI_BASE}/groups/{WORKSPACE_ID}/reports/{template_id}/Clone"
    headers = {
    ...
    session.max_redirects = 0  # stop automatic redirect
    try:
        r = session.post(base_url, headers=headers, json=body, timeout=30, allow_redirects=False)
    except requests.exceptions.ConnectTimeout:
        raise RuntimeError("Power BI Clone API timed out. Region mismatch or firewall block.")
    except Exception as e:
        raise RuntimeError(f"Clone report error: {str(e)}")
    # handle redirect manually (retry)
    if r.status_code in (301, 302, 307, 308):
        region_url = r.headers.get("Location")
        if not region_url:
            raise RuntimeError("Redirect returned without valid region URL.")
        try:
            r = session.post(region_url, headers=headers, json=body, timeout=30)
        except requests.exceptions.ConnectTimeout:
            raise RuntimeError(
                f"Could not reach Power BI region endpoint: {region_url}. "
                "Your network may be blocking regional cluster."
            )
    if r.status_code >= 300:
        raise RuntimeError(f"Report clone failed: {r.status_code} - {r.text[:1000]}")
    return r.json()
...
def execute_model_creation(
    ...
    parts = build_parts_from_ado(new_table, columns=columns)
    model_id = upload_parts_to_fabric(fabric_token, model_display, parts)
    # wait a bit for dataset creation to appear
    time.sleep(5)
    dataset_id = find_pbi_dataset(pbi_token, model_display)
    if not dataset_id:
        raise RuntimeError("Dataset not found after model upload")
    clone_res = clone_pbi_report(pbi_token, template_report_id, dataset_id, report_name)
    return {
        "model_id": model_id,
        "dataset_id": dataset_id,
...
def create_model():
    try:
        ...
        log.exception("create_model unexpected error")
        return jsonify({"error": f"Unexpected error: {str(e)}", "success": False}), 500
